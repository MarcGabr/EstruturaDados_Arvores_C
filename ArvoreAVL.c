...
//Função que calcula qual e maior
int maior(int a, int b){
    if(a>b)return a; return b;
}
//Função que devine qual o tamanho da altura de uma folha
int altura(arvore *a){
    if(a==NULL)return -1;
    return(a->altura);
}
//Função que balanceia
void balanceia(arvore **a){
    ...
    if((altura((*a)->esq))- altura((*a)->dir)==2){
            //Apos entrar ele testa se o resultado for maior que  e rotação para direita se for menor ou igual rotdupla esquerda
    ...
    //se a altura esquerda menos a altura direita for igual a -2  ela esta ou desbalanceada para esquerda ou dupla direita
        ...
        if((altura((*a)->esq))- altura((*a)->dir)==-2){
                //Apos entrar ela testa se o resultado for menor que 1 e rotação para a esquerda e se for maior ou igual rotdupladireita
